# WCM v0.15 → DDC-WCM v0.2 Mapping
...
## Verifier-bound validity in v0.2

The v0.2 mapping distinguishes manifest normalization from WCM verification.

`map-wcm` maps manifest policy and identity fields but leaves
`wcm.verification_result` as `UNKNOWN`.

The reference `verify-and-map` workflow executes the upstream WCM verifier with
explicit trusted public-key files, computes the exact manifest digest, records
the verifier identity/version and executable digest, records trusted-key and
report digests, and binds that evidence directly to the mapped bundle.

The reference CLI does not accept an operator-supplied `VALID` flag.
